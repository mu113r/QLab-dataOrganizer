...
def cbodies(wb_in, wb_out, current_row):
    ...
    ws_in = wb_in['Cell Bodies Summary']
    start_row = 2
    while(ws_in.cell(row=start_row, column=3).value != None):
        ws_out.cell(row=current_row, column=2, value=ws_in.cell(row=start_row, column=3).value)
        ws_out.cell(row=current_row, column=3, value=ws_in.cell(row=start_row, column=4).value)
        current_row += 1
        start_row += 1
    return wb_out, current_row
...
def build_output(list_of_input_path):
    ...
    for each_input in list_of_input_path:
        ...
        wb_out, current_row = cbodies(wb_in, wb_out, current_row)
        #current_row += 1
